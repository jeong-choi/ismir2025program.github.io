...
def doesMeetingExist(accessToken, tokenExpireTime, accountCreds, key_type, key_val):
    ...
    accessToken, tokenExpireTime, meetingList = getListOfMeetings(
        accessToken, tokenExpireTime, accountCreds
    )
...
def createWebinar(
    accessToken, tokenExpireTime, accountCreds, topic, startTime, duration
):
    accessToken, tokenExpireTime = checkToken(
        accessToken, tokenExpireTime, accountCreds
    )
...
def addPanelistsToWebinar(
    accessToken, tokenExpireTime, accountCreds, webinarId, panelistList
):
    """
    ...
    panelists = [{"email": email, "name": name} for (email, name) in panelistList]
    resp = requests.post(apiEndpointUrl, data=json.dumps(panelists), headers=headers)
    return accessToken, tokenExpireTime, resp.json()
...
def deletePanelistFromWebinar(
    accessToken, tokenExpireTime, accountCreds, webinarId, panelistId
):
    accessToken, tokenExpireTime = checkToken(
        accessToken, tokenExpireTime, accountCreds
    )
